...
import { Loader2, PartyPopper, Download, Save } from "lucide-react"; // Added Save icon
import type { Questionnaire, Question as QuestionType, Section as SectionType } from "@/types";
import { useToast } from "@/hooks/use-toast";
import logo from "@/assets/logo.png";

// --- MOCK DATA FOR DEMO PAGE (unchanged) ---
...
// --- Reusable Components (FormField, SidebarSection, etc. are unchanged) ---
...
const SidebarSection = ({ section, index, scrollToElement }: { section: SectionType; index: number; scrollToElement: (id: string) => void }) => {
    ...
    return (
        <div className="space-y-1">
            <button type="button" onClick={() => { setIsExpanded(!isExpanded); scrollToElement(`section-${section.id}`); }} className="w-full text-left px-3 py-2.5 text-sm font-medium hover:bg-accent rounded-md transition-colors flex items-center justify-between group border border-transparent hover:border-primary/20" >
            ...
            {isExpanded && (
                <div className="ml-3 space-y-0.5 border-l-2 border-muted pl-3 py-1">
                    {section.questions.map((question, qIndex) => ( <button key={question.id} type="button" onClick={() => scrollToElement(`question-${question.id}`)} className="w-full text-left px-2 py-1.5 text-xs text-muted-foreground hover:text-foreground hover:bg-accent/50 rounded transition-colors" > Question {qIndex + 1} </button> ))}
                </div>
            )}
        </div>
    );
...
// --- MAIN COMPONENT (SIMPLIFIED LOGIC) ---
const ClientForm = () => {
    ...
    // --- Session ID Management (unchanged) ---
    useEffect(() => {
    ...
    // --- Fetch Questionnaire & Load Progress (unchanged) ---
    ...
    // --- NEW: Manual Save Progress Logic ---
    const saveProgressMutation = useMutation({
        mutationFn: async (answers: Record<string, any>) => {
            if (!sessionId || !questionnaireId || isDemoMode) {
            ...
            if (Object.keys(nonEmptyAnswers).length === 0) {
                toast({ title: "Nothing to Save", description: "Please answer at least one question to save progress.", variant: "default" });
                return;
            }
    ...
    const handleSaveProgress = () => {
        ...
        saveProgressMutation.mutate(formValues);
    };

    // --- Final Submit Logic (unchanged) ---
    ...
    const onSubmit = (data: Record<string, any>) => {
        ...
        if (sessionId) {
            finalSubmitMutation.mutate({ questionnaire_id: questionnaireId!, respondent_id: sessionId, answers: nonEmptyAnswers });
        } else {
            toast({ title: "Error", description: "Session ID is missing. Cannot submit.", variant: "destructive" });
        }
    };

    // --- Helper functions (handleDownloadPdf, scrollToElement) are unchanged ---
    const handleDownloadPdf = async () => {
        ...
        try {
            const doc = new jsPDF();
            const pageHeight = doc.internal.pageSize.height; const pageWidth = doc.internal.pageSize.width; const margin = 20; const contentWidth = pageWidth - (margin * 2); let yPos = 20;
            const logoResponse = await fetch(logo); const logoBlob = await logoResponse.blob(); logoUrl = URL.createObjectURL(logoBlob);
            const logoWidth = 30; const logoHeight = 10; doc.addImage(logoUrl, 'PNG', margin, yPos, logoWidth, logoHeight);
            doc.setFontSize(16); doc.setFont('helvetica', 'bold'); doc.text(activeQuestionnaire.title, pageWidth - margin, yPos, { align: 'right' });
            doc.setFontSize(10); doc.setFont('helvetica', 'normal'); doc.text(activeQuestionnaire.organization, pageWidth - margin, yPos + 6, { align: 'right' });
            yPos += Math.max(logoHeight, 12) + 10; doc.setLineWidth(0.5); doc.line(margin, yPos, pageWidth - margin, yPos); yPos += 15;
            activeQuestionnaire.sections?.forEach((section) => {
                if (yPos > pageHeight - 30) { doc.addPage(); yPos = margin; }
                doc.setFontSize(12); doc.setFont('helvetica', 'bold'); doc.setTextColor(80); doc.text(section.title, margin, yPos); yPos += 8;
                section.questions.forEach((question) => {
                    if (submittedData[question.id]) {
                        if (yPos > pageHeight - 20) { doc.addPage(); yPos = margin; }
                        doc.setFontSize(11); doc.setFont('helvetica', 'bold'); const qText = doc.splitTextToSize(`Q: ${question.prompt}`, contentWidth); doc.text(qText, margin, yPos); yPos += (qText.length * 5) + 2;
                        if (yPos > pageHeight - 20) { doc.addPage(); yPos = margin; }
                        doc.setFontSize(11); doc.setFont('helvetica', 'normal'); doc.setTextColor(0); const aText = doc.splitTextToSize(`A: ${submittedData[question.id]}`, contentWidth - 5); doc.text(aText, margin + 5, yPos); yPos += (aText.length * 5) + 10;
                    }
                });
            });
            ...
            console.error(err);
            toast({ title: "Failed to generate PDF", description: err.message || "An unknown error occurred.", variant: "destructive" });
        } finally {
            ...
            <form onSubmit={methods.handleSubmit(onSubmit)} className="min-h-screen bg-surface">
                ...
                <div className="mx-auto max-w-7xl px-6 py-8 flex gap-8">
                    <aside className="hidden lg:block w-64 shrink-0 sticky top-24 self-start">
                        <div className="border rounded-lg bg-background overflow-hidden" style={{ maxHeight: 'calc(100vh - 120px)' }}><div className="p-4 border-b bg-muted/30"><h3 className="font-semibold text-sm">Form Navigation</h3></div><nav className="overflow-y-auto p-3 space-y-1" style={{ maxHeight: 'calc(100vh - 180px)' }}>{activeQuestionnaire.sections?.map((section, index) => ( <SidebarSection key={section.id} section={section} index={index} scrollToElement={scrollToElement} /> ))}</nav></div>
                    </aside>

                    <main className="flex-1 max-w-4xl">
                        <div className="space-y-16">
                            {activeQuestionnaire.sections?.map((section, index) => ( <FormSectionComponent key={section.id} section={section} sectionNumber={index + 1} /> ))}
                            {(!activeQuestionnaire.sections || activeQuestionnaire.sections.length === 0) && ( <div className="text-center p-8 border rounded-lg"><p className="text-muted-foreground">This form has no questions yet.</p></div> )}
                        </div>
                    </main>
                </div>
